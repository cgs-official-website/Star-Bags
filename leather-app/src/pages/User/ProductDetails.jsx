...
function ProductDetails() {
  ...
  const averageRatingScore = totalReviewsCount > 0 
  ...
  const getStarPercentFactor = (starNum) => totalReviewsCount > 0 ? `${(getStarCountFactor(starNum) / totalReviewsCount) * 100}%` : "0%";
  ...
  const handleWriteReviewSubmit = (rating, text) => {
    ...
    alert("Thank you thalaiva! Your review has been updated in the dataset cleanly.");
  };
  ...
  return (
    <div className="product-details-page" style={{ position: "relative" }}>
      ...
      <div className="container py-4">
        <div className="row mb-4">
          ...
          <div className="col-lg-6 ps-lg-5 d-flex justify-content-between align-items-center">
            ...
            <div className="text-danger fs-4 cursor-pointer wishlist-btn" onClick={() => toggleWishlist(currentProduct)}>
        ...
        <div className="row mb-5">
          <div className="col-lg-6 mb-4 mb-lg-0">
            <div className="main-image-container">
              ...
              <img key={currentImageIndex} src={mainImage} alt={currentProduct.name} className="main-product-image mx-4" />
            ...
            <div className="thumbnail-gallery mt-4">
              {images.map((img, idx) => (
                <div key={idx} className={`thumbnail-box ${currentImageIndex === idx ? "active" : ""}`} onClick={() => setCurrentImageIndex(idx)}>
          ...
          <div className="col-lg-6 ps-lg-5">
            ...
            <div className="d-flex align-items-center mb-3">
              <div className="rating-stars text-warning me-2">
                {[...Array(5)].map((_, i) => i < Math.round(Number(averageRatingScore)) ? <FaStar key={i} /> : <FaRegStar key={i} style={{ color: "#d1d5db" }} />)}
              </div>
              <span className="rating-count" style={{ fontSize: "0.85rem", color: "#6b7280" }}>
                {totalReviewsCount > 0 ? `${averageRatingScore} / 5.0` : "No ratings yet"} ({totalReviewsCount} Reviews)
              </span>
            </div>
              ...
              <div className="size-selector-section mb-4">
                ...
                <div className="d-flex gap-3 flex-wrap">
                  {sizeOptions.map((opt, idx) => (
                    <div key={idx} className={`size-option-box ${selectedSize === opt.value ? "selected" : ""}`} onClick={() => setSelectedSize(opt.value)}>
            ...
            <button className="btn-buy-now" onClick={handleProceedToCheckoutDirectly}>Buy Now</button>

            <div className="delivery-box mt-4">
              <div className="d-flex justify-content-between align-items-center mb-2">
                <h6 className="mb-0 fw-bold text-dark">Delivery Address</h6>
                {savedAddresses.length > 0 && (
                  <h6 className="cursor-pointer" style={{ color: "#8b5cf6", fontSize: "13px", fontWeight: "600" }} onClick={() => setIsModalOpen(true)}>
              ...
              {savedAddresses.length === 0 ? (
                <div className="empty-address-viewport d-flex justify-content-center align-items-center py-2" style={{ backgroundColor: "#ffffff" }}>
                  <button type="button" className="add-delivery-trigger-btn d-flex align-items-center gap-2" style={{ background: "none", border: "1px dashed #8b5cf6", color: "#8b5cf6", padding: "6px 12px", borderRadius: "6px", fontSize: "0.85rem", fontWeight: "600" }} onClick={() => navigate("/address")}>
                    <TiPencil style={{ transform: "rotate(-45deg)" }} /> Add your Delivery Address
                  </button>
                </div>
              ) : !activeSelectedAddress ? (
                <div className="p-2 text-warning small fw-semibold">⚠️ Please click 'Choose Address' to select a card.</div>
              ) : (
                <div className="text-muted mt-2" style={{ fontSize: "0.9rem" }}>
                  <p className="mb-1"><strong>{activeSelectedAddress.name}</strong>, {activeSelectedAddress.address}</p>
                  <p className="mb-1">{activeSelectedAddress.city}, {activeSelectedAddress.state} - {activeSelectedAddress.pin}</p>
        ...
        <div className="details-section mb-5">
          ...
          <div className="table-responsive">
            <table className="table table-bordered product-table">
              ...
              <tbody>
                <tr><td>Product Name</td><td>{currentProduct.name}</td></tr>
                <tr><td>Brand Name</td><td>{currentProduct.brandName || "Krish Leather Factory"}</td></tr>
                <tr><td>Product ID</td><td>{currentProduct.productId || "SBP-BAG-00003"}</td></tr>
                <tr><td>Material</td><td>{currentProduct.material || "Genuine Leather, Premium Inner Lining"}</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="reviews-section">
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <button
              ...
              style={{ backgroundColor: "#8b5cf6", borderRadius: "6px", fontSize: "0.8rem", border: "none" }}
            >
              Write a Review
            </button>
          </div>
          ...
          <div className="overall-rating-card shadow-sm mb-4">
            <div className="rating-score">
              <h2>
                {averageRatingScore} {totalReviewsCount > 0 ? <FaStar className="fs-4 text-warning" /> : <FaRegStar className="fs-4" />}
              </h2>
            ...
            <div className="rating-bars">
              {[5, 4, 3, 2, 1].map((star) => (
                <div key={star} className="rating-bar-row">
                  <span>{star} <FaStar style={{ color: getStarCountFactor(star) > 0 ? "#fbbf24" : "#e5e7eb" }} /></span>
                  <div className="progress-custom">
                    <div
                      className="progress-bar-fill"
                      style={{ width: getStarPercentFactor(star), backgroundColor: star >= 3 ? "#22c55e" : star === 2 ? "#fbbf24" : "#ef4444" }}
                    />
                  </div>
          ...
          <div className="filter-buttons">
            {["All", "Positive", "Negative"].map((type) => (
              <button key={type} className={`filter-btn-user ${activeFilter === type ? "active" : ""}`} onClick={() => setActiveFilter(type)}>
                {type}
              </button>
            ))}
          </div>
          ...
          {sortedFilteredReviews.length === 0 ? (
            <div className="text-center py-5 shadow-sm border rounded-3 my-3 bg-white" style={{ fontFamily: "system-ui" }}>
              <div style={{ fontSize: "2.8rem", color: "#a8a29e" }} className="mb-2">✍️</div>
              <h5 className="fw-bold mb-1 text-dark" style={{ fontSize: "1.1rem" }}>No matching reviews yet</h5>
              <p className="text-muted small mb-3">Be the first to leave a verified review matching your selected filter metrics!</p>
            </div>
          ) : (
            <div 
            ...
            >
              <div className="row flex-nowrap flex-md-wrap g-4 mt-2 reviews-row" style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                {sortedFilteredReviews.map((review) => (
                  <div key={review.id} className="col-10 col-sm-6 col-md-4 review-col flex-shrink-0">
                    <div className="review-card p-3 border rounded bg-white shadow-sm">
                      <div className="review-header d-flex justify-content-between mb-2">
                        <div className="reviewer-info d-flex align-items-center gap-2">
                          ...
                          <p className="reviewer-name fw-bold m-0 small" style={{ color: "#111827" }}>{review.name}</p>
                        </div>
                        <div className="rating-stars text-warning small">
                          {[...Array(5)].map((_, i) => i < review.rating ? <FaStar key={i} /> : <FaRegStar key={i} />)}
                        </div>
                      </div>
                      <p className="review-text small text-muted" style={{ lineHeight: "1.5", height: "65px", overflowY: "auto" }}>{review.text}</p>
                      
                      <div className="review-actions d-flex justify-content-between align-items-center mt-3 border-top pt-2">
                        <span className="text-muted" style={{ fontSize: "0.7rem" }}>{review.date}</span>
                        <div className="helpful-btns d-flex gap-2">
                          <button style={{ color: "#058aff", background: "none", border: "none", fontSize: "16px" }} onClick={() => handleFeedback(review.id, "like")}>
                          ...
                          <button style={{ color: "#f25858", background: "none", border: "none", fontSize: "16px" }} onClick={() => handleFeedback(review.id, "dislike")}>
        ...
        <div className="recent-products-outer-container py-4 border-top mt-5" style={{ backgroundColor: "#fafafa" }}>
      ...
      {isModalOpen && (
        <div className="address-popup-modal-overlay d-flex justify-content-center align-items-center" style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.45)", backdropFilter: "blur(2px)", zIndex: 999 }}>
          <div className="address-popup-modal-box shadow-lg bg-white p-4 rounded" style={{ width: "92%", maxWidth: "625px", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div className="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
              <h4 className="m-0 text-dark fw-bold" style={{ fontSize: "1.4rem" }}>Select delivery address</h4>
              <button type="button" style={{ background: "transparent", border: "none", cursor: "pointer" }} onClick={() => setIsModalOpen(false)}>
                <MdClose size={24} />
              </button>
            </div>
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h5 className="m-0 text-dark fw-semibold" style={{ fontSize: "1.05rem" }}>Saved Addresses</h5>
              <button type="button" className="popup-add-new-address-btn d-flex align-items-center gap-1" style={{ backgroundColor: "#8b5cf6", color: "#fff", border: "none", padding: "7px 14px", borderRadius: "6px", fontWeight: "600", fontSize: "0.8rem" }} onClick={() => navigate("/address")}>
                <MdAdd size={16} /> Add a New Address
              </button>
            </div>
            <div className="popup-address-items-scroller pr-1" style={{ maxHeight: "365px", overflowY: "auto" }}>
              {savedAddresses.map((addr, index) => (
                <div key={addr.id} className="popup-address-item-card p-3 mb-3 border rounded position-relative" onClick={() => { setSelectedAddressId(addr.id); setIsModalOpen(false); }} style={{ cursor: "pointer", borderColor: selectedAddressId === addr.id ? "#8b5cf6" : "#e5e7eb", backgroundColor: selectedAddressId === addr.id ? "#f5f3ff" : "#fff" }}>
                  <div className="d-flex align-items-center gap-2 mb-2">
                    <span className="fw-bold text-dark" style={{ fontSize: "1rem" }}>Address {index + 1}</span>
                    {selectedAddressId === addr.id && <span className="px-2 py-0.5 rounded-pill" style={{ backgroundColor: "#ddd6fe", color: "#6d28d9", fontSize: "0.72rem", fontWeight: "700" }}>Selected</span>}
                    <div className="popup-card-actions-tray d-flex align-items-center gap-2 ms-auto" style={{ position: "absolute", top: "14px", right: "14px" }}>
                      <button type="button" className="edit-icon" style={{ background: "transparent", border: "1px solid #c4b5fd", color: "#8b5cf6", borderRadius: "20px", padding: "3px 12px", fontSize: "0.78rem" }} onClick={(e) => { e.stopPropagation(); navigate("/address"); }}>
                        <MdModeEdit size={14} /> <span style={{ marginLeft: "2px" }}>Edit</span>
                      </button>
                      <button type="button" style={{ background: "transparent", border: "none", color: "#9ca3af", cursor: "pointer" }} onClick={(e) => promptDeleteAddress(e, addr.id)}>
                        <FaTrashAlt size={13} />
                      </button>
                    </div>
                  </div>
                  <p className="text-dark m-0 mb-1" style={{ fontSize: "0.88rem" }}>{addr.name} , {addr.address}</p>
                  <p className="text-dark m-0 mb-1" style={{ fontSize: "0.88rem" }}>{addr.city}, {addr.state} - {addr.pin}</p>
                  <p className="text-muted m-0" style={{ fontSize: "0.85rem" }}>Mobile: {addr.mobile}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {showDeleteModal && (
        <div className="modal-overlay-custom" style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div className="modal-box-custom shadow-lg bg-white p-4 rounded text-center" style={{ width: "90%", maxWidth: "400px" }}>
            <h5 className="fw-bold mb-2">Confirm Deletion</h5>
            <p className="text-muted small">Are you sure you want to delete this delivery address profile?</p>
            <div className="d-flex gap-3 mt-4">
              <button type="button" className="btn btn-secondary flex-fill" onClick={() => setShowDeleteModal(false)}>Cancel</button>
              <button type="button" className="btn btn-danger flex-fill" onClick={executeDeleteAddress}>Delete</button>
            </div>
          </div>
        </div>
      )}
