...
function TrackOrder() {
  ...
  // Extract realtime order payload safely
  ...
  useEffect(() => {
    if (!currentUser || !order?.id) return;
  ...
  // ─── SAFE ADDRESS RESOLVER (NO DEFAULT FALLBACK VALUES) ───
  const getLivePlacedBillingSpecs = () => {
    const fallbackAddress = { name: "", address: "", cityAndPin: "", mobile: "", method: "Cash on delivery" };

    if (!order) return fallbackAddress;

    const details = order.customerDetails || {};
    const methodStr = order.paymentMode || order.paymentMethod || "Cash on delivery";
    const formattedMethod = methodStr.toLowerCase().trim() === "cod" ? "Cash on delivery" : "Online payment";
  ...
  // ─── CRASH PROOF FIXED TIMELINE STEPS ARRAY ───
  const activeOrderSteps = [
    {
      id: 1,
      label: "Order placed",
      desc: `Your order has been logged into the system schema on ${order?.time || "processing date"}.`,
      done: true,
    },
    {
      id: 2,
  ...
  const trackGrandTotal = Number(order?.discountedPrice) || 0;
  const savings = itemsPrice > trackGrandTotal ? itemsPrice - trackGrandTotal : 0;
  
  const calculatedSubTotal = itemsPrice - savings;
  const calculatedGstAmount = Math.round(calculatedSubTotal * 0.18);
      ...
      <div className="container to-page py-4">
        ...
        <div className="to-main-grid">
              ...
              <div className="to-product-info d-flex gap-3">
                ...
                <div className="to-product-details flex-grow-1">
                  ...
                  <div className="to-product-price-row d-flex align-items-baseline gap-2 mb-2">
                    <span className="to-product-price fw-bold text-dark" style={{ fontSize: "1.15rem" }}>₹{trackGrandTotal}</span>
                    {itemsPrice > trackGrandTotal && (
                  ...
                  <div key={step.id} className={`to-step d-flex gap-3 ${step.done ? "done" : "pending"}`}>
                    ...
                    <div className="to-step-content pb-3">
                      <div className="to-step-label fw-bold text-dark" style={{ fontSize: "0.95rem" }}>
                        {step.label}
                        {idx === 0 && order?.time && <span className="to-step-date text-muted font-monospace small fw-normal">&nbsp;— {order.time}</span>}
                      </div>
          ...
          <div className="to-right d-flex flex-column gap-3">
            ...
            <div className="to-card border bg-white">
              ...
              <div className="to-address-block">
                <p className="to-address-sub-title fw-bold text-secondary small mb-2 d-flex align-items-center gap-1">
                  <TbTruckDelivery className="to-addr-icon text-purple" /> Shipping Address
                </p>
                
                <p className="to-addr-name fw-bold text-dark m-0" style={{ fontSize: "1.05rem", color: "#0f172a" }}>
                  {billingSpecs?.name || ""}
                </p>
                <p className="to-addr-line text-muted small mt-1 mb-0" style={{ fontSize: "0.88rem", color: "#475569" }}>
                  {billingSpecs?.address || ""}
                </p>
                <p className="to-addr-line text-muted small m-0" style={{ fontSize: "0.88rem", color: "#475569" }}>
                  {billingSpecs?.cityAndPin || ""}
                </p>
                {billingSpecs?.mobile && (
                  <p className="to-addr-line small m-0 mt-1 fw-semibold" style={{ fontSize: "0.88rem", color: "#1e293b", display: "flex", alignItems: "center", gap: "6px" }}>
                    <span style={{ fontWeight: "bold", opacity: 0.6 }}>MOBILE :</span> {billingSpecs.mobile}
                  </p>
                )}
                ...
                <div className="to-delivery-meta d-flex flex-column gap-2 mt-3">
                  <span className="to-meta-badge bg-light border rounded px-3 py-1.5 small fw-semibold text-dark" style={{ textTransform: "capitalize" }}>
                    <MdCreditCard className="me-1 text-purple" /> Method: {billingSpecs?.method || "Cash on delivery"}
                  </span>
            ...
            {/* ORDER SUMMARY BLOCK */}
            <div className="to-card border bg-white">
              <h6 className="to-section-title fw-bold mb-3 text-dark">Order Summary</h6>
