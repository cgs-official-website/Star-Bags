...
const WishlistHeart = () => {
  ...
  return (
    <div className="wishlist">
      ...
      >
        {isWishlist ? <FaHeart className="text-danger" /> : <FiHeart className="text-danger" />}
      </button>
    </div>
  );
};
          ...
          <div className="ProductCard-section my-3 ">
              <div className="container d-flex gap-3 flex-nowrap">
                {productCard.map((pro, index) => (
                  <div className="card border-0 shadow-sm" key={index} style={{width: "15rem"}}>
                    <img src={pro.image} className="card-img-top" alt="..." />
                    <WishlistHeart />
                    <div className="card-body">
                      <div className="d-flex justify-content-between pt-2">
                        <h6 className="card-title">{pro.name}</h6>
                        <span className="rating-stars d-flex align-items-center" style={{color:'black'}} ><FaStar className="me-1" style={{color:'#fff240'}}/> {pro.rating}</span> 
                      </div>
